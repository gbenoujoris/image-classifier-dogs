...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        for line in infile:
            # Process line by striping newline and splitting by comma (if any)
    ...
    for key in results_dic:
        ...
        if pet_label in dognames_dic:
            results_dic[key].append(1)  # Pet label is a dog
        else:
            results_dic[key].append(0)  # Pet label is not a dog

        # Check if classifier label is a dog
        if classifier_label.split(',')[0] in dognames_dic:
